...
def process_random_data(
    ...
    # 循环计算船只pr
    for ship_id in ship_ids:
        for battle_type in ['pvp_solo','pvp_div2','pvp_div3']:
            ship_data = processed_data.get(ship_id)[battle_type]
            # 用户数据
            account_data = [
                ship_data['battles_count'],
...
def process_ranked_data(
    ...
    for season_number, season_data in overall_data['seasons'].items():
        formatted_data['data_list'].append(
            BaseFormatData.format_user_rank_processed_data(
                algo_type = algo_type,
                processed_data = season_data,
                season_number = season_number
            )
        )
        formatted_data['data_len'] += 1

    for battle_type in ['overall', 'ship_type', 'data_len', 'data_list']:
        result[battle_type] = formatted_data[battle_type]
    return JSONResponse.get_success_response(result)
